...
# Generate base x values from 0.0 to 3.9 in steps of 0.1
base_x_values = np.arange(0.0, 3.3, 0.1)  # 4.0 to include 3.9, 40 values
num_rows = len(base_x_values)  # 40 rows
num_cols = 10  # Second decimal places: 0.00 to 0.09

# Get the directory of the script and construct the output file path
script_dir = os.path.dirname(__file__)
output_file = os.path.join(script_dir, 'erf_table.html')

# Start building the HTML table
html = '<table class="erf-table">\n'
html += '    <thead>\n'
html += '        <tr>\n'
html += '            <th class="base-col">\( \\boldsymbol{x} \\)</th>\n'  # Base x value column with border
...
for i in range(num_rows):
    ...
    # Remaining columns: erf(x + 0.01 * j)
    for j in range(num_cols):
        x = base_x + j * 0.01
        erf_value = erf(x)
        html += f'            <td>\\( {erf_value:.5f} \\)</td>\n'
    html += '        </tr>\n'
